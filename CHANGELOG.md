...
- `ForgeCredoChecks.NoTelemetryAssertionsInTest`: builds its event context once per file instead of once per visited AST node, which makes the check linear in file size rather than quadratic. The context was passed inside the prewalk's capture (`&traverse(&1, &2, issue_meta, event_context(source_file, params))`), and a capture evaluates its arguments on every call, so `event_context/2` — which walks the whole file itself via `attribute_events/1` — was re-run for every node. Measured on a consumer's suite of 1296 files, `mix credo --strict` drops from 1411s to 43s with an unchanged verdict; the other checks each traverse that same tree in 2-3s. The defect had no behavioural signature, so all 352 existing tests passed either way; the added test drives 800 clauses and asserts a wall-clock bound, sized against a measured 14.6s quadratic versus tens of milliseconds linear.
- `ForgeCredoChecks.UnsupervisedSpawn`: now flags `spawn_opt` (local, `Process.` and `:erlang.` spellings), `Process.spawn_link`, `Process.spawn_monitor` and `Process.spawn_opt` — previously only `Process.spawn` was matched, so the same call under a sibling name passed — and `Task.start/1,3`, which was explicitly carved out before. `Task.start` is the one Task entry point that neither links nor attaches to a supervisor: it reads as part of the supervised abstraction and is a raw spawn. `Task.Supervisor.start_child/2` and `Task.async`/`async_nolink` remain unflagged, and `Task.start_link` is deliberately left alone because it links to its caller and inherits that process's supervision.
