...
- `ForgeCredoChecks.TelemetryControlFlow`: flags `:telemetry.attach`/`attach_many` with an inline anonymous-function handler that performs control flow (`send/2`, `:erlang.send/2`, `GenServer.call`/`cast`) instead of observation-only recording. A raising telemetry handler is auto-detached by the runtime, so using the bus to deliver load-bearing signals fails silently; the idiomatic mechanism is `Phoenix.PubSub`. Named MFA-tuple handlers are out of scope (documented in "Known limits"). Supports `:excluded_paths` as a shrinking migration bridge; `# credo:disable-for-next-line` is honored for intentional exceptions.
