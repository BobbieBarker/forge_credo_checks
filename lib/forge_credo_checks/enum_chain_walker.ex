defmodule ForgeCredoChecks.EnumChainWalker do
  ...
  def nil_predicate?({:&, _, [{:/, _, [{:is_nil, _, _}, 1]}]}), do: true

  def nil_predicate?({:&, _, [{:==, _, [{:&, _, [1]}, nil]}]}), do: true

  def nil_predicate?({:&, _, [{:===, _, [{:&, _, [1]}, nil]}]}), do: true

  def nil_predicate?({:fn, _, [{:->, _, [[{name, _, ctx1}], {:is_nil, _, [{name, _, ctx2}]}]}]})
      when is_atom(name) and is_atom(ctx1) and is_atom(ctx2),
      do: true

  def nil_predicate?({:fn, _, [{:->, _, [[{name, _, ctx1}], {op, _, [{name, _, ctx2}, nil]}]}]})
      when op in [:==, :===] and is_atom(name) and is_atom(ctx1) and is_atom(ctx2),
      do: true

  def nil_predicate?(_), do: false
end
